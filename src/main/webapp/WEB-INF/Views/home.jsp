...
<a href="myBookings" class="hover:text-blue-400 transition">
...
<a href="./logout" class="bg-red-500 px-4 py-2 rounded-lg hover:bg-red-600 transition shadow" href="./logout">
...
<div class="backdrop-blur-xl bg-white/10 border border-white/20 p-10 rounded-3xl shadow-2xl w-[950px]">

<h2 class="text-white text-3xl font-semibold mb-6">
<i class="fa-solid fa-magnifying-glass"></i> Search Flights
</h2>

<form action="searchFlight" onsubmit="return validateSearch()" method="post" class="grid grid-cols-5 gap-5">

<input type="text" name="source" placeholder="From"
class="p-3 rounded-lg bg-white/80 focus:outline-none focus:ring-2 focus:ring-blue-400" autofocus="autofocus"/>

<input type="text" name="destination" placeholder="To"
class="p-3 rounded-lg bg-white/80 focus:outline-none focus:ring-2 focus:ring-blue-400"/>
...
<button class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg hover:scale-105 transition font-semibold shadow-lg flex items-center justify-center gap-2">
...
<i class="fa-solid fa-plane"></i> Available Flights
</h2>
...
<div id="toast"
     class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 
            ...
            text-lg font-semibold hidden z-50 transition">Please fill all fields</div>
...
function validateSearch(){
    ...
    let source = document.querySelector("input[name='source']").value.trim();
    let destination = document.querySelector("input[name='destination']").value.trim();
    let date = document.querySelector("input[name='date']").value;
    let passengers = document.querySelector("input[name='passengers']").value;
    ...
    if(passengers <= 0){
        showToast("⚠️ Enter valid passengers");
        return false;
    }

    if(source.toLowerCase() === destination.toLowerCase()){
        showToast("⚠️ Source and destination cannot be same");
        return false;
    }
